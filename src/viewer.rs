    ...
    pub fn num_bytes(&self) -> Byte {
        match self {
            ...
            UpstreamMessage::SendAgentPrompt(request) => {
                // Count prompt, inline attachments, and the encoded user query
                // exactly as transported, without interpreting the protobuf.
                let prompt_bytes: Byte = request.prompt.len().into();
                let user_query_bytes: Byte = request
                    .user_query_b64
                    .as_ref()
                    .map_or(0, String::len)
                    .into();
                ...
                prompt_bytes
                    ...
                    .and_then(|bytes| bytes.add(user_query_bytes))
